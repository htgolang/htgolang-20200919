...
func ModifyUser(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		if err != nil {
			fmt.Println(err)
			http.Redirect(w, r, fmt.Sprintf("/modify?id=%v&name=%v&sex=%v&tel=%v&brithday=%v&addr=%v&passwd=%v&err=%v", user.ID, user.Name, user.Sex, user.Tel, user.Brithday, user.Addr, user.Passwd, err.Error()), 302)
			return
		}
