...
func (u *UserController) Prepare() {
	...
	u.Data["navkey"] = filepath.Base(u.Ctx.Input.URL())
}
...
func (u *UserController) Password() {
	...
	if u.Ctx.Input.IsPost() {
		nuser := new(models.LoginUser)
		err := u.ParseForm(nuser)
		if err != nil {
			errmsg = err
		} else {
			_, err := server.AuthLogin(nuser)
			if err != nil {
				errmsg = errors.New("密码错误")
			} else {
				user.PassWord = nuser.NewPassWord
				err := server.ModifyUser(user)
				if err != nil {
					errmsg = errors.New("修改密码失败")
				} else {
					u.Redirect("/user/list", 301)
				}
			}
